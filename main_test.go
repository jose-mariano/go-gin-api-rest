...
func TestVerificaRespostaDaSaudacaoComParametro(t *testing.T) {
	r := SetupDasRotasDeTeste()
	r.GET("/:nome", controllers.Saudacao)

	req, _ := http.NewRequest("GET", "/jose", nil)
	resposta := httptest.NewRecorder()
	r.ServeHTTP(resposta, req)

	assert.Equal(t, http.StatusOK, resposta.Code)
	mockDaResposta := `{"API diz":"E ai jose, tudo beleza?"}`
...
func TestBuscaAlunoPofCPFHandler(t *testing.T) {
	database.ConectaComBancoDeDados()
	CriaAlunoMock()
	defer DeletaAlunoMock()
...
func TestEditaUmAlunoHandler(t *testing.T) {
	...
	aluno := models.Aluno{Nome: "Aluno Teste Junior", CPF: "47123456789", RG: "123456701"}
